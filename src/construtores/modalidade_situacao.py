"""Constrói o dataset de modalidade × situação."""
...
def construir(df: pd.DataFrame) -> pd.DataFrame:
    """
    Gera o dataset cruzado de modalidade e situação.
    ...
    return (
        df.groupby(["modalidade", "status_simplificado"]).size().reset_index(name="qtd")
    )
